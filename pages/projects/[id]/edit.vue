...
const { data: project, status: projectStatus } = await useAsyncData(
    `project_${id}`,
    () => $fetch(`/api/projects/${id}`),
);
const { data: years } = await useAsyncData("fiscal_years", () =>
    $fetch("/api/master/fiscal-years"),
);
const { data: quarters } = await useAsyncData("quarters", () =>
    $fetch("/api/master/quarters"),
);
const { data: categories } = await useAsyncData("categories", () =>
    $fetch("/api/master/categories"),
);
const { data: agencies } = await useAsyncData("agencies", () =>
    $fetch("/api/master/agencies"),
);
const { data: responsiblePersons } = await useAsyncData(
    "responsible_persons",
    () => $fetch("/api/master/responsible-persons"),
);
const { data: budgetSources } = await useAsyncData("budget_sources", () =>
    $fetch("/api/master/budget-sources"),
);
const { data: statuses } = await useAsyncData("project_statuses", () =>
    $fetch("/api/master/statuses"),
);
...
const selectedStatusName = computed(() => {
    ...
    const found = (statusList as any[]).find(
        (s) => s.id === Number(form.value.statusId),
    );
    ...
    (newVal) => {
        if (newVal) {
            form.value = {
                ...
                implementationDate: newVal.implementationDate
                    ? newVal.implementationDate.split("T")[0]
                    : null,
                completionDate: newVal.completionDate
                    ? newVal.completionDate.split("T")[0]
                    : null,
                initialBudget:
                    Number(newVal.initialBudget) || Number(newVal.budget) || 0,
                actualBudget: Number(newVal.actualBudget) || 0,
                budget: Number(newVal.budget) || 0,
                description: newVal.description || "",
            };
            displayInitialBudget.value = formatNumber(
                form.value.initialBudget.toString(),
            );
            displayActualBudget.value = formatNumber(
                form.value.actualBudget.toString(),
            );
        }
    },
    { immediate: true },
    ...
    try {
        ...
        await $fetch(`/api/projects/${id}`, {
            ...
            body: {
                ...
                quarterId: form.value.quarterId
                    ? Number(form.value.quarterId)
                    : null,
                categoryId: Number(form.value.categoryId),
                agencyId: Number(form.value.agencyId),
                responsibleId: Number(form.value.responsibleId),
                budgetSourceId: form.value.budgetSourceId
                    ? Number(form.value.budgetSourceId)
                    : null,
                statusId: Number(form.value.statusId),
                completionDate:
                    selectedStatusName.value === "ดำเนินการเสร็จสิ้น"
        ...
        toast.add({
            title: "สำเร็จ",
            description: "อัปเดตข้อมูลโครงการเรียบร้อยแล้ว",
            color: "success",
        });
        await navigateTo("/");
    } catch (e: any) {
        toast.add({
            title: "เกิดข้อผิดพลาด",
            description: e.statusMessage || "ไม่สามารถอัปเดตข้อมูลได้",
            color: "error",
        });
    } finally {
        loading.value = false;
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <UFormField label="ปีงบประมาณ" required>
                        <select
                            ...
                            class="w-full h-10 px-3 rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 text-sm focus:ring-2 focus:ring-primary focus:outline-none"
                        >
                    ...
                    <UFormField label="ไตรมาส">
                        <select
                            v-model="form.quarterId"
                            class="w-full h-10 px-3 rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 text-sm focus:ring-2 focus:ring-primary focus:outline-none"
                        >
                            <option value="">-- เลือกไตรมาส (ถ้ามี) --</option>
                            <option
                                v-for="q in quarters as any[]"
                                :key="q.id"
                                :value="q.id"
                            >
                                {{ q.name }}
                    ...
                    <UFormField label="ผู้รับผิดชอบ" required>
                        ...
                        >
                            ...
                            <option
                                v-for="r in responsiblePersons as any[]"
                                :key="r.id"
                                :value="r.id"
                            >
                                {{ r.name }}
                    ...
                    <UFormField label="แหล่งที่มาของงบประมาณ">
                        <select
                            v-model="form.budgetSourceId"
                            class="w-full h-10 px-3 rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 text-sm focus:ring-2 focus:ring-primary focus:outline-none"
                        >
                            <option value="">
                                -- เลือกแหล่งที่มาของงบประมาณ --
                            </option>
                            <option
                                v-for="bs in budgetSources as any[]"
                                :key="bs.id"
                                :value="bs.id"
                            >
                                {{ bs.name }}
                        ...
                        <UFormField label="วันที่เสร็จสิ้นโครงการ">
                            <UInput
                                ...
                                :ui="{
                                    ...
                                    icon: {
                                        leading: { wrapper: 'text-green-600' },
                                    },
                                }"
                            />
                        </UFormField>
                        ...
                        <UInput
                            ...
                            @update:model-value="
                                (val) => {
                                    ...
                                    displayInitialBudget =
                                        formatNumber(sanitized);
                                }
                            "
                            required
                    ...
                    >
                        <UTextarea
                            ...
                            placeholder="ระบุรายละเอียดโครงการ (ถ้ามี)..."
                            :rows="8"
                            class="w-full"
                        />
                    </UFormField>
                    ...
                    <div
                        v-if="(project as any)?.files?.length > 0"
                    ...
                    >
                        ...
                        <div class="grid grid-cols-1 gap-2">
                            <div
                                v-for="file in (project as any).files"
                                :key="file.id"
                                class="flex flex-col p-3 border rounded bg-gray-50 dark:bg-gray-800 gap-1"
                            >
                                <div class="flex items-center justify-between">
                                    <div
                                    ...
                                    >
                                        ...
                                        <a
                                            :href="`/api/projects/files/${file.uuid}`"
                                            target="_blank"
                                            class="text-xs text-blue-600 hover:underline truncate font-medium"
                                            >{{ file.filename }}</a
                                        >
                                    </div>
                                    <div class="flex items-center gap-2">
                                        <span
                                            class="text-[10px] text-gray-400 mr-2"
                                            >{{
                                ...
                                <div
                                    v-if="file.note"
                                    class="text-[11px] text-gray-500 italic mt-1 flex items-start gap-1"
                                >
                                    <UIcon
                    ...
                    <UModal
                        ...
                        title="แก้ไขหมายเหตุ/บันทึก"
                        @update:open="(val) => !val && (editingFile = null)"
                    >
                        <template #content>
                            <div
                                class="p-8 space-y-6 flex flex-col items-center"
                            >
                                <div
                                ...
                                >
                                    <UIcon
                                        name="i-heroicons-pencil-square"
                                        class="w-12 h-12 text-primary mx-auto opacity-20"
                                    />
                                    <h3 class="text-lg font-bold">
                                        บันทึก/หมายเหตุเพิ่มเติม
                                    </h3>
                                    <UFormField
                                        label="ระบุข้อมูลที่ต้องการแก้ไข"
                                        class="text-left"
                                    >
                                        <UTextarea
                                            v-if="editingFile"
                                            v-model="editingFile.note"
                                            placeholder="พิมพ์หมายเหตุสำหรับไฟล์นี้..."
                                            :rows="4"
                                            class="w-full"
                                        />
                                    </UFormField>
                                ...
                                <div
                                    class="flex justify-center gap-3 w-full border-t pt-6"
                                >
                                    <UButton
                                        label="ยกเลิก"
                                        color="gray"
                        ...
                        <div class="flex items-center justify-center w-full">
                            ...
                            >
                                ...
                                >
                                    ...
                                    />
                                    <p
                                        class="text-sm text-gray-500 dark:text-gray-400 font-semibold text-center"
                                    >
                                        คลิกเพื่ออัปโหลด หรือลากไฟล์มาวางที่นี่
                                    </p>
                                    <p class="text-[10px] text-gray-400 mt-1">
                                        PDF, DOCX, JPG (ขนาดไม่เกิน 30MB)
                                    </p>
                                </div>
                                <input
                                    type="file"
                        ...
                        >
                            ...
                            <ul class="space-y-3">
                                <li
                                    ...
                                    class="flex flex-col gap-3 p-4 border rounded-md bg-white dark:bg-gray-900 dark:border-gray-700 shadow-sm"
                                >
                                    <div
                                    ...
                                    <UInput
                                        v-model="item.note"
                                        placeholder="เพิ่มบันทึก/หมายเหตุ (ไม่บังคับ)"
                                        size="sm"
                                        icon="i-heroicons-pencil-square"
                                    />
                                </li>
