...
watch(
    ...
    (newStatuses) => {
        if (newStatuses && !form.value.statusId) {
            const receivedStatus = (newStatuses as any[]).find(
                (s: any) => s.name === "รับเอกสาร",
            );
...
const submitProject = async () => {
    ...
    try {
        ...
        toast.add({
            title: "สำเร็จ",
            description: "เพิ่มโครงการและอัปโหลดไฟล์เรียบร้อยแล้ว",
            color: "success",
        });
        await navigateTo("/");
    } catch (e: any) {
        toast.add({
            ...
            description: e.statusMessage || "ไม่สามารถเพิ่มโครงการได้",
            color: "error",
        });
    } finally {
        loading.value = false;
